...
SJ Trading 是一個使用 Python 開發的量化交易回測與模擬框架，旨在提供一個模組化、易於擴展的平台，幫助開發者與研究員測試與驗證交易策略。
...
在執行應用程式之前，您需要設定必要的環境變數。

1.  **複製範本檔案**
    專案提供了一個 `.env` 範本。請先複製一份：
    ```bash
    cp .env.example .env
    ```
    *注意：如果專案中沒有 `.env.example`，您可以直接建立一個新的 `.env` 檔案。*

2.  **填入您的設定**
    打開 `.env` 檔案，並填入您自己的金鑰或設定。**請勿將您的敏感資訊提交到版本控制中。**
    ```dotenv
    # .env

    API_KEY="您的 API Key"
    SECRET_KEY="您的 Secret Key"
    CA_CERT_PATH="/您的/憑證/路徑.pfx"
    CA_PASSWORD="您的憑證密碼"
    ```
